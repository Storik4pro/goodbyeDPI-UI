...
from PySide6.QtCore import QObject, QTimer, Signal, Slot, QByteArray, QDateTime, QSharedMemory, qCritical, QLocale, \
    QTranslator
...
class StarterImpl(QObject):
    ...
    def init(self, locale=None):
        ...
        js_function = """(function (path) { return Qt.resolvedUrl(path,null).toString();})"""
    ...
    def writeData(self):
        ...
        if byte_array.size() > 1024:
            qCritical(f"Data size is {byte_array.size()} bytes, which exceeds the limit of 1024 bytes.")
            return
        data = self.__shared_memory.data()
        if data is not None:
            shared_memory_data = memoryview(self.__shared_memory.data())
            shared_memory_data[:self.__shared_memory.size()] = bytearray(self.__shared_memory.size())

...
class PatchDownloadWorker(QObject):
    ...
    def _process_patch(self, patch_file, mode: Literal['manual', 'auto'] = 'auto'):
        ...
        try:
            ...
            if not os.path.exists(requirements_path) or not os.path.exists(goodbyeDPI_UI_path):
    ...
    def _download_and_process_dependency(self, url, dependency_patch_path):
        download_update(url, dependency_patch_path, self.progressChanged, debug_check=False)
    ...
    def _apply_patch(self, patch_dir):
        ...
        for i, patch_url in enumerate(self.processed_patches):
            if not os.path.exists(os.path.join(DEBUG_PATH+DIRECTORY, patch_url, "goodbyeDPI UI")):
        ...
        for root, _, files in os.walk(goodbyeDPI_UI_path):
            for file in files:
                source_path = os.path.relpath(os.path.join(root, file), goodbyeDPI_UI_path)
                target_path = os.path.join(old_directory, os.path.relpath(root, goodbyeDPI_UI_path), 
                                           file)
                ...
                if os.path.exists(_file) and source_path != ".":
                    ...
                    shutil.copy(os.path.join(DEBUG_PATH+DIRECTORY, source_path), target_path)
        ...
        self.preparationProgress.emit()
        shutil.make_archive(zip_file_path.replace('.zip', ''), 'zip', f'{DIRECTORY}unpacked_patch')
        try:
            for i, patch_url in enumerate(self.processed_patches):
                shutil.rmtree(self.processed_patches[i])
        except Exception as ex:

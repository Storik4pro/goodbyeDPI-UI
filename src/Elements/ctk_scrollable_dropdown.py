...
class CTkScrollableDropdown(customtkinter.CTkToplevel):
    ...
    def __init__(self, attach, x=None, y=None, button_color=None, height: int = 200, width: int = None,
        ...
        if scrollbar is False:
            ...
            self.frame._scrollbar.grid_forget()
        else:
            self.frame._scrollbar.grid_forget()
    ...
    def _init_buttons(self, **button_kwargs):
        ...
        for row in self.values:
            ...
            self.widgets[self.i].pack(fill="x", pady=(1, 0), padx=(0, 4))
            self.i+=1
        if self.scrollbar:
            self.frame._scrollbar.configure(width=5, fg_color='transparent', bg_color='transparent')
            self.frame._scrollbar.place(relx=1, rely=0, relheight=1, anchor='ne')
            self.frame._scrollbar.lift(self.frame)

            self.frame._scrollbar.lower(self.frame)

            self.frame.bind("<Enter>", lambda e: self._show_scrollbar())
            self.frame.bind("<Leave>", lambda e: self._hide_scrollbar())
    ...
    def _show_scrollbar(self):
        self.frame._scrollbar.lift(self.frame)
        self.frame._scrollbar.configure(button_color='gray')

    def _hide_scrollbar(self):
        self.frame._scrollbar.lower(self.frame)
        self.frame._scrollbar.configure(fg_color='transparent', button_color='#404040')
    ...
    def place_dropdown(self):
        ...
        if self.resize:
            if self.button_num<=5:      
                self.height_new = (self.button_height+5)* self.button_num
        ...
        if string:
            ...
            for key in self.widgets.keys():
                ...
                if not similar:
                    ...
                    self.widgets[key].pack(fill="x", pady=(1, 0), padx=(0, 4))
                    i+=1
                    
            if i==1:
                self.no_match.pack(fill="x", pady=(1, 0), padx=(0, 4))
            else:
